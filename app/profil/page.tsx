...
export default async function Page() {
  ...
  return (
    <div
      className="min-h-screen flex flex-col font-mono t-text"
      style={{
        ...
        <main className="flex-1 w-full max-w-[1400px] mx-auto px-4 sm:px-8 py-8">
          <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            <div className="t-card border p-4 lg:col-span-2">
              <h1 className="text-2xl sm:text-3xl font-black uppercase tracking-tighter mb-1">
                Mein Profil
              </h1>
              <p className="text-xs t-muted uppercase tracking-widest">
                Account verwalten
              </p>
            </div>
            ...
            <section className="t-card border p-5 sm:p-6 space-y-3">
              <h2 className="text-xs font-bold uppercase tracking-widest t-muted mb-2">
                Accountdaten
              </h2>
              {user.username && (
                <div className="flex justify-between text-sm gap-4">
                  <span className="t-muted uppercase text-xs tracking-widest shrink-0">
                    Benutzername
                  </span>
              ...
              <div className="flex justify-between text-sm">
                <span className="t-muted uppercase text-xs tracking-widest">
                  Email bestätigt
                </span>
              ...
              {isAdmin && (
                <div className="flex justify-between text-sm items-center">
                  ...
                  <a
                    href="/admin"
                    className="text-purple-400 font-bold uppercase tracking-widest hover:underline"
                  >
                    Admin →
                  </a>
                </div>
            ...
            <section className="t-card border p-5 sm:p-6">
              <h2 className="text-xs font-bold uppercase tracking-widest t-muted mb-4">
                Bestellhistorie
              </h2>
              {userOrders.length === 0 ? (
                <p className="text-xs t-faint uppercase tracking-widest">
                ...
                <div className="space-y-3">
                  {userOrders.map((order) => (
                    <div
                      key={order.id}
                      className="border t-border-s p-3 text-xs"
                    >
                      <div className="flex justify-between mb-1">
                        <span className="t-muted uppercase tracking-widest">
                          Bestellung #{order.id}
                        </span>
                      ...
                      <div className="flex justify-between">
                        <span className="uppercase tracking-widest t-faint">
                          {order.status}
                        </span>
            ...
            <section className="t-card border p-5 sm:p-6">
              ...
              <a
                href="/belege"
                className="inline-block border t-border py-2 px-4 text-xs uppercase tracking-widest hover:bg-white hover:text-black transition font-bold t-text"
              >
                → Belege einsehen
              </a>
            </section>
            ...
            <section className="t-card border p-5 sm:p-6">
              ...
              {completedChallenges.length === 0 ? (
                ...
                <div className="space-y-2">
                  {completedChallenges.map((c) => (
                    <div
                      key={c.challenge.id}
                      className="flex justify-between text-xs border t-border-s p-3"
                    >
                      <div>
                        <p className="t-text uppercase tracking-widest">
                          {c.challenge.title}
                        </p>
              ...
              <a
                href="/shop/challenges"
                className="inline-block mt-4 text-xs underline hover:t-text transition t-muted"
              >
            ...
            <section className="border border-red-900 bg-black/80 p-5 sm:p-6 lg:col-span-2">
              ...
              <p className="text-xs t-muted mb-4">
                Das Löschen deines Accounts ist unwiderruflich. Alle Daten
                werden permanent entfernt.
              </p>
              <DeleteAccountButton />
            </section>
          </div>
