...
export default async function HomePage() {
  ...
  const [posts, userCountResult, countdownSetting, upcomingDrops] =
    await Promise.all([
      db
        .select()
        .from(homePosts)
  ...
  if (!countdown.enabled) {
    ...
    if (nextDrop) {
      countdown = {
        enabled: true,
        targetDate: new Date(nextDrop).toISOString().slice(0, 16),
        ...
        <section className="w-full max-w-[1400px] mx-auto px-4 sm:px-8 md:px-16 py-16 md:py-24 lg:flex lg:items-center lg:justify-between lg:gap-12">
          <div className="max-w-2xl">
            ...
            <p className="text-sm text-zinc-400 leading-relaxed mb-8 max-w-md">
              Streetwear ohne Kompromisse. Jedes Piece ist streng limitiert —
          ...
          {countdown.enabled && (
            <div className="lg:shrink-0">
              <p className="text-center text-[10px] uppercase tracking-widest text-zinc-500 mb-3">
                <span className="text-white font-bold">{userCount}</span> Aktive
                User
              </p>
          ...
          {posts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {posts.map((post) => (
                <article
                  key={post.id}
                  className="border border-zinc-700 bg-black/80 flex flex-col hover:border-zinc-500 transition-all group"
                >
                  {post.imageUrl && (
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <h3 className="text-base font-black uppercase tracking-tight text-white leading-tight flex-1">
                      {post.title}
                    </h3>
                    {post.body && (
          ...
          <div className="max-w-[1400px] mx-auto px-4 sm:px-8 md:px-16 py-12 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
              <h2 className="text-2xl font-black uppercase tracking-tighter text-white">
                Keinen Drop verpassen.
              </h2>
              <p className="text-xs text-zinc-500 mt-1 uppercase tracking-widest">
                Discord beitreten für Early-Access und Previews
              </p>
            </div>
            <a
