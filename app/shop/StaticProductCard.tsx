...
export default function StaticProductCard() {
  ...
  const stock = 10; // TODO: aus DB laden
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center px-4 py-12 md:p-24 gap-8">
      ...
      <div className="relative w-full max-w-[380px] t-card border-[3px] t-border p-5 sm:p-6"
        ...
        <div className="flex justify-between items-center mb-4 text-[10px] uppercase tracking-widest">
          ...
          {soldOut ? (
            ...
            <span className="border border-green-800 px-2 py-1 text-green-400 font-bold">
              Noch {stock} Stück
            </span>
          )}
        </div>
        ...
        {/* SOLD OUT Overlay */}
        {soldOut && (
          <div className="absolute inset-0 z-20 flex items-center justify-center overflow-hidden pointer-events-none rounded-sm">
        ...
        <div className="t-bg mb-5 overflow-hidden border t-border-s">
          <img
        ...
        <p className="text-lg sm:text-xl font-black t-text mb-4 border-l-4 border-red-600 pl-3">{product.price}</p>

        {soldOut ? (
          <button onClick={() => setShowNewsletter(true)}
            className="block w-full py-4 text-center uppercase font-black tracking-[0.2em] border-[3px] border-red-500 text-red-400 hover:bg-red-600 hover:text-white transition-all text-sm">
            Next Drop beitreten
          </button>
        ) : (
          <a href={`/shop/product/${product.id}`}
            className="block w-full py-4 text-center uppercase font-black tracking-[0.2em] border-[3px] t-border t-text hover:bg-white hover:text-black transition-all text-sm">
            Details ansehen
          </a>
        )}
      </div>
      ...
      <a href="https://discord.gg/T4RwVJRyRp" target="_blank" rel="noopener noreferrer"
        className="flex items-center gap-3 border t-border px-5 py-3 t-card hover:border-[#5865F2] hover:text-[#5865F2] transition-all text-xs font-bold uppercase tracking-widest t-text">
