...
import RedeemCodeButton, {
  type RedeemCodeButtonHandle,
} from "@/app/shop/components/RedeemCodeButton";
            ...
            <div className="fixed top-0 right-0 z-[401] h-full w-[80vw] sm:w-[340px] lg:w-[22vw] bg-black border-l border-zinc-700 flex flex-col font-mono t-invert">
              <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
                <Link
                  href="/"
                  onClick={() => setOpen(false)}
                  className="text-xl font-black uppercase tracking-tighter italic text-white hover:opacity-70 transition"
                >
                  BELLATOR.
                </Link>
                <button
                  onClick={() => setOpen(false)}
                  className="border border-zinc-600 p-2 hover:border-white transition text-white"
                >
                  <svg
                    width="16"
                    height="16"
  ...
  return (
    <>
      ...
      >
        {hasUnreadChat && (
          <span
            className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-white border border-black"
            aria-label="Neue Nachrichten"
          />
        )}
